 ...
 */
class AshsControllerOrders extends JControllerAdmin
{
	/**
	 * Proxy for getModel.
	 * @since	1.6
	 */
	public function getModel($name = 'order', $prefix = 'AshsModel', $config = array())
	{
	...
	public function export(){
		...
		if($export_choose == 'selected'){
			...
			$sql = "SELECT * FROM #__hikashop_order";
			$db->setQuery($sql);
			$items = $db->loadObjectList();
		}
		...
		$outputpath = JPATH_ROOT.'/media/com_ashs/files/';
		...
		foreach($items as $tmp){		 
			...
			$billTO = $model->billTO($tmp->order_billing_address_id); 
			$shipTO = $model->shipTO($tmp->order_shipping_address_id);
			
			$order = $model->getOrder($tmp->order_id);
			
			$products = $model->getProduct($tmp->order_id); 
			...
			<table class="table table-bordered" width="4400px">
				<?php
					...
					/* }
						...
						echo '<tr><td colspan="10">'.JText::_('COM_ASHS_NO_COLUMN_SELECTES').'</td></tr>';
					}  */
				?>
			</table>
			</div>
		<?php }
			...
			$downfilename = JURI::root().'media/com_ashs/files/'.$filename;
